interface SvgData {
  color: string;
  activeColor: string;
}

const LogoIndex = (): JSX.Element => {
  return (
    <svg
      width="360"
      height="76"
      viewBox="0 0 360 76"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0)">
        <path
          d="M154.522 33.5139L149.306 58.7967H135.718L140.934 33.5139C141.343 31.5267 141.106 31.1604 139.18 31.1604H134.275C132.362 31.1604 131.961 31.5267 131.556 33.4634C131.556 33.4634 131.556 33.4971 131.556 33.5139L126.34 58.7967H112.74L117.96 33.5139C118.369 31.5267 118.132 31.1604 116.202 31.1604H111.297C109.367 31.1604 108.979 31.5309 108.57 33.5139L103.35 58.7967H89.7618L97.77 19.9862H108.807L110.557 23.3208C111.563 22.2294 112.782 21.3684 114.133 20.7939C115.483 20.2193 116.937 19.9442 118.398 19.9862H121.214C126.528 19.9862 130.113 22.3651 131.397 26.3859C133.772 21.9146 137.214 19.9862 141.371 19.9862H144.192C152.176 19.9862 156.235 25.2954 154.522 33.5139Z"
          fill="white"
        />
        <path
          d="M195.961 33.5139L193.537 45.269C191.841 53.4875 185.586 58.7967 177.594 58.7967H167.301C159.321 58.7967 155.262 53.4833 156.959 45.2648L159.383 33.5139C161.075 25.2954 167.33 19.9862 175.326 19.9862H185.635C193.598 19.9862 197.662 25.2954 195.961 33.5139ZM182.361 33.5139C182.77 31.5266 182.532 31.1603 180.607 31.1603H175.702C173.772 31.1603 173.384 31.5308 172.975 33.5139L170.547 45.2648C170.138 47.2521 170.379 47.6184 172.305 47.6184H177.21C179.135 47.6184 179.528 47.2479 179.937 45.2648L182.361 33.5139Z"
          fill="white"
        />
        <path
          d="M280.764 30.4235C280.267 32.6298 279.067 34.6011 277.355 36.0232L270.434 41.9176C268.775 43.3548 266.687 44.1631 264.519 44.208H254.937L254.72 45.248C254.312 47.2352 254.549 47.6015 256.478 47.6015H277.212L274.906 58.7799H251.458C243.479 58.7799 239.42 53.4665 241.116 45.248L243.54 33.497C245.237 25.2785 251.491 19.9694 259.483 19.9694H269.801C277.76 19.9862 281.876 25.0175 280.764 30.4235ZM266.53 33.5139C266.939 31.5266 266.702 31.1603 264.773 31.1603H259.867C257.946 31.1561 257.537 31.5308 257.128 33.5139L256.311 37.56H264.405C264.768 37.5445 265.116 37.4074 265.396 37.1696C265.676 36.9317 265.874 36.6061 265.958 36.2422L266.53 33.5139Z"
          fill="white"
        />
        <path
          d="M296.041 52.2076C295.289 55.8495 292.497 58.8009 288.274 58.8009C284.051 58.8009 282.481 55.8537 283.234 52.2076C283.986 48.5615 286.778 45.6185 291.001 45.6185C295.223 45.6185 296.781 48.5615 296.041 52.2076Z"
          fill="#4CD62B"
        />
        <path
          d="M325.196 19.9862L322.89 31.1561L317.167 58.7925H303.583L309.306 31.1561H302.524L304.834 19.982L325.196 19.9862ZM315.107 12.7403C312.752 10.0457 313.472 5.42283 316.705 2.42089C319.939 -0.581053 324.472 -0.82525 326.839 1.87355C329.206 4.57235 328.474 9.19525 325.241 12.1972C322.007 15.1991 317.465 15.4433 315.107 12.7403Z"
          fill="white"
        />
        <path
          d="M347 25.5733L342.937 45.2649C342.528 47.2521 342.765 47.6184 344.691 47.6184H353.14L350.835 58.7967H339.679C331.699 58.7967 327.636 53.4834 329.333 45.2649L333.396 25.5733H328.257L330.563 14.395H335.714L337.602 5.24182H351.203L349.314 14.395H360.008L357.69 25.5733H347Z"
          fill="white"
        />
        <path
          d="M36.7338 5.69226H53.9765L39.6321 75.9999H22.3894L36.7338 5.69226Z"
          fill="white"
        />
        <path
          d="M62.0337 5.69226H79.2764L68.1001 60.7797H50.8533L62.0337 5.69226Z"
          fill="white"
        />
        <path
          d="M11.1804 5.69226H28.4231L17.2468 60.7797H0L11.1804 5.69226Z"
          fill="white"
        />
        <path
          d="M246.279 19.9862L221.085 58.7967H207.187L197.997 19.9862H212.881L216.883 45.4711L231.395 20.0073L246.279 19.9862Z"
          fill="white"
        />
      </g>
      <defs>
        <clipPath id="clip0">
          <rect width="360" height="76" fill="white" />
        </clipPath>
      </defs>
    </svg>
  );
};

const HomeSideBar = ({ color, activeColor }: SvgData): JSX.Element => {
  return (
    <svg
      width="112"
      height="56"
      viewBox="0 0 112 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M44 24.0001L56 14.6667L68 24.0001V38.6667C68 39.374 67.7191 40.0522 67.219 40.5523C66.7189 41.0524 66.0406 41.3334 65.3334 41.3334H46.6667C45.9594 41.3334 45.2811 41.0524 44.781 40.5523C44.281 40.0522 44 39.374 44 38.6667V24.0001Z"
        stroke={color}
        strokeWidth="2.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M52 41.3333V28H60V41.3333"
        stroke={color}
        strokeWidth="2.5"
        strokeLinecap="round"
        strokeLinejoin="round"
      />
      <path
        d="M0 0V0C2.20914 0 4 1.79086 4 4V52C4 54.2091 2.20914 56 0 56V56V0Z"
        fill={activeColor}
      />
    </svg>
  );
};

const AwardSideBar = ({ color, activeColor }: SvgData): JSX.Element => {
  return (
    <svg
      width="112"
      height="56"
      viewBox="0 0 112 56"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0)">
        <path
          d="M56.0001 32.0003C61.1548 32.0003 65.3334 27.8216 65.3334 22.667C65.3334 17.5124 61.1548 13.3337 56.0001 13.3337C50.8454 13.3337 46.6667 17.5124 46.6667 22.667C46.6667 27.8216 50.8454 32.0003 56.0001 32.0003Z"
          stroke={color}
          strokeWidth="2.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
        <path
          d="M50.9466 30.5199L49.3333 42.6665L55.9999 38.6665L62.6666 42.6665L61.0533 30.5066"
          stroke={color}
          strokeWidth="2.5"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
      </g>
      <path
        d="M0 0V0C2.20914 0 4 1.79086 4 4V52C4 54.2091 2.20914 56 0 56V56V0Z"
        fill={activeColor}
      />
      <defs>
        <clipPath id="clip0">
          <rect
            width="32"
            height="31.9998"
            fill="white"
            transform="translate(40 12.0002)"
          />
        </clipPath>
      </defs>
    </svg>
  );
};

const LogoSideBar = (): JSX.Element => {
  return (
    <svg
      width="3rem"
      height="2.625rem"
      viewBox="0 0 48 42"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <path
        d="M22.2416 0H32.6816L23.9964 42H13.5564L22.2416 0Z"
        fill="#5965E0"
      />
      <path
        d="M37.56 0H48.0001L41.2331 32.9078H30.7905L37.56 0Z"
        fill="#5965E0"
      />
      <path
        d="M6.76945 0H17.2095L10.4425 32.9078H0L6.76945 0Z"
        fill="#5965E0"
      />
    </svg>
  );
};

const GithubIndex = (): JSX.Element => {
  return (
    <svg
      width="40"
      height="40"
      viewBox="0 0 40 40"
      fill="none"
      xmlns="http://www.w3.org/2000/svg"
    >
      <g clipPath="url(#clip0)">
        <path
          d="M20 0.833374C31.05 0.833374 40 9.63338 40 20.4867C40 29.1717 34.27 36.5367 26.325 39.1334C25.325 39.3184 24.9583 38.71 24.9583 38.1884C24.9583 37.7217 24.975 36.485 24.9833 34.8467C30.5467 36.0317 31.72 32.21 31.72 32.21C32.63 29.9417 33.945 29.335 33.945 29.335C35.7567 28.1167 33.805 28.1417 33.805 28.1417C31.7967 28.2784 30.7417 30.1667 30.7417 30.1667C28.9583 33.1717 26.06 32.3034 24.9167 31.8017C24.7367 30.53 24.2217 29.665 23.65 29.1734C28.0917 28.6817 32.76 26.9917 32.76 19.4617C32.76 17.3167 31.985 15.5634 30.7017 14.1884C30.9267 13.6917 31.6017 11.6934 30.5267 8.98671C30.5267 8.98671 28.8517 8.46004 25.0267 11.0017C23.4267 10.565 21.7267 10.3484 20.0267 10.3384C18.3267 10.3484 16.6267 10.565 15.0267 11.0017C11.2267 8.46004 9.55168 8.98671 9.55168 8.98671C8.47668 11.6934 9.15168 13.6917 9.35168 14.1884C8.07668 15.5634 7.30168 17.3167 7.30168 19.4617C7.30168 27.0117 11.9767 28.6734 16.4267 29.1567C15.7267 29.7467 15.0767 30.9517 15.0767 32.7934C15.0767 35.4234 15.1017 37.5367 15.1017 38.175C15.1017 38.69 14.7517 39.305 13.7267 39.1084C5.72501 36.5284 1.52588e-05 29.1584 1.52588e-05 20.4867C1.52588e-05 9.63338 8.95501 0.833374 20 0.833374Z"
          fill="#B3B9FF"
        />
      </g>
      <defs>
        <clipPath id="clip0">
          <rect
            width="40"
            height="40"
            fill="white"
            transform="matrix(-1 0 0 1 40 0)"
          />
        </clipPath>
      </defs>
    </svg>
  );
};

export { HomeSideBar, AwardSideBar, LogoSideBar, LogoIndex, GithubIndex };
